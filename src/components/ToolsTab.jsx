...
import { PLAN, WEEKS } from "../plan.js";
...
function buildTrackingPrompt() {
  ...
  return [
    ...
    ...WEEKS.map(w => {
      ...
      const days = DOW.map(d => w.workouts[d] || "Rest").join(" / ");
...
export function ToolsTab({onSync, syncing}) {
  ...
  return <div style={{padding:16,display:"flex",flexDirection:"column",gap:12}}>
    <div style={cardSt}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:15,fontWeight:600,color:"#1A1A1A",marginBottom:4}}>Rebuild summary</div>
          <p style={{fontSize:13,color:"#888",margin:0,lineHeight:1.5}}>Re-reads all entry files and rebuilds the summary index. Run if dashboard or plan dots look stale.</p>
        </div>
        <button onClick={onSync} disabled={syncing} style={{flexShrink:0,marginLeft:16,padding:"8px 16px",background:syncing?"#F5F3EF":"#1A1A1A",color:syncing?"#BBB":"#fff",border:"none",borderRadius:10,fontSize:13,fontWeight:500,cursor:syncing?"default":"pointer",display:"flex",alignItems:"center",gap:6}}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{animation:syncing?"spin 1s linear infinite":"none"}}><path d="M21 12a9 9 0 1 1-2.636-6.364M21 3v6h-6"/></svg>
          {syncing ? "Rebuilding…" : "Rebuild"}
        </button>
      </div>
    </div>
    {!promptText
